...
export class ComentariosComponent implements OnInit {
  ...
  exportCsv() {
    const rows = this.comentariosTable.map((item) => [
      item.id,
      item.comment,
      item.enabled === true ? 'Si' : 'No',
      item.client_name,
  ...
  exportPdf() {
    ...
    autoTable(doc, {
      ...
      body: this.comentariosTable.map((item) => [
        item.id,
        item.comment,
        item.enabled === true ? 'Si' : 'No',
        item.client_name,
  ...
  saveTestimonial() {
    ...
    if (this.addRegister === true) {
      ...
      this.apiService
        .updateLandingTestimonials(this.currentId, request)
        .subscribe({
          next: ({ data }) => {
            this.getTestimonials();
            this.visible = false;
          ...
          error: (err) =>
            console.error('Error al actualizar testimonial:', err),
        });
    }
  }
